...
class StrategicAgent(ExpertAgent):
    ...
    async def handle_task(
        ...
        try:
            ...
            state.artifacts["strategic_context"] = (
    ...
    def _determine_context(self, prompt: str) -> str:
        ...
        if any(
            ...
            for keyword in ["agent", "ai agent", "intelligent agent"]
        ):
            return "ai_agent_development"
        elif any(
            keyword in prompt_lower for keyword in ["workflow", "automation", "process"]
        ):
            return "workflow_automation"
        elif any(
            keyword in prompt_lower
            for keyword in ["documentation", "docs", "knowledge"]
        ):
            return "documentation_system"
        elif any(
            keyword in prompt_lower for keyword in ["code", "generation", "programming"]
        ):
            return "code_generation"
        elif any(
            keyword in prompt_lower
            for keyword in ["research", "analysis", "investigation"]
        ):
            return "research_analysis"
        else:
            return "general_strategy"

    def _get_framework_guidance(
        ...
        specific_guidance = context_specific_guidance.get(
            context,
            [
                "Apply strategic thinking to the problem",
                "Consider long-term implications",
            ],
        )

        return {**base_guidance, "context_specific": specific_guidance}
    ...
    async def _create_strategic_response(
        ...
        enhanced_prompt = build_enhanced_prompt(
            agent_role=AgentRole.STRATEGIC_EXPERT, state=temp_state
        )
        ...
        if strategy_results:
            response_parts.extend(["### 🎲 Strategic Principles", ""])
        ...
        response_parts.extend(
            ["### 🏗️ Framework Guidance", "", "**Playing to Win Framework:**"]
        )

        for principle in framework_guidance.get("playing_to_win", []):
            response_parts.append(f"- {principle}")

        response_parts.extend(["", "**Context-Specific Recommendations:**"])
