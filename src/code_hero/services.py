...
from .types import LoggerType, StateManagerType, SupervisorType
    ...
    for name, service in services:
        try:
            if not isinstance(service, ServiceInterface):
                raise ServiceError(
                    f"Service {name} does not implement ServiceInterface"
                )

            health = await service.check_health()
            statuses.append(
                ServiceStatus(name=name, status=Status.RUNNING, details=health)
            )
        except Exception as e:
            statuses.append(
                ServiceStatus(name=name, status=Status.FAILED, error=str(e))
            )
